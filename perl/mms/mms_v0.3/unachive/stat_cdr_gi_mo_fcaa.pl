############################################################################
#Desc: stat cdr gi mo fail per cause
#Usage:stat_cdr_gi_mo_fcaa.pl [start_date] [end_date]
...
sub daily_dispose
{
	...
	$sql = "delete from TSMD_CDR_GI_MO_FCAA" 
	...
	$sql = "select C.area_code,B.Mmse_result, A.statuscode, count(*)" 
		...
		. " where (A.receivetime between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
		. " and (B.start_time between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
		. " and A.cdr_type =0 and B.cdr_type=0 and remoteno=66"
		. " and mmse_result in (128,130,135,136,-1)"
		. " and A.STATUSCODE IN  ('6000','6003','6001','6607','6300')" 
		...
		. " and substr(to_char(B.calling),1,7) = substr(to_char(C.start_gt),3,7)" 
	...
	for (@$stat_area_total_array)
	{
		...
		my $sql = "insert into TSMD_CDR_GI_MO_FCAA(START_TIME,AREA_CODE,RESULT,MMS_STATUS_CODE,"
	...
	$sql = "select MMS_STATUS_CODE,area_code,sum(FCAA_NUM) from TSMD_CDR_GI_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
	...
	$stat_row_hash = {};
	$sql = "select area_code,sum(FCAA_NUM) from TSMD_CDR_GI_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
		. " group by area_code";
	...
	$stat_row_hash = {};
	$sql = "select MMS_STATUS_CODE,RESULT,sum(FCAA_NUM) from TSMD_CDR_GI_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
	...
	$sql = "select MMS_STATUS_CODE,sum(FCAA_NUM) from TSMD_CDR_GI_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
	...
	# stat total fcaa num
	$stat_row_hash = {};
	$sql = "select sum(FCAA_NUM) from TSMD_CDR_GI_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))";
...
sub stat_rate_by_area
{
	...
	foreach my $key (keys %$stat_row_hash)
	{
		...
		if($total == 0 )
		{
			$sql = "update TSMD_CDR_GI_MO_FCAA set RATIO_IN_TOTAL = 0"
    				. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
				. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
		...
		{
			$sql = "update TSMD_CDR_GI_MO_FCAA set RATIO_IN_TOTAL = (FCAA_NUM / $total) * 1.0000"
    				. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
				. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
				. " and AREA_CODE=$areacode and MMS_STATUS_CODE= $gdrtype and NE_TYPE=$ne_type";
...
sub stat_avg_rate_by_area
{
	...
	foreach my $key (keys %$stat_row_hash)
	{
		...
		if($total == 0 )
		{
			$sql = "update TSMD_CDR_GI_MO_FCAA set AVG_RATIO_IN_TOTAL = 0"
    				. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
				. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
		...
		{
			$sql = "update TSMD_CDR_GI_MO_FCAA set AVG_RATIO_IN_TOTAL = (FCAA_NUM / $total) * 1.0000"
    				. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
				. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
				. " and AREA_CODE=$key and NE_TYPE=$ne_type";
...
sub update_title
{
	...
	$sql = "update TSMD_CDR_GI_MO_FCAA a" 
