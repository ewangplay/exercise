############################################################################
#Desc: stat mmsc misc per cause
#Usage:stat_mmsc_misc_per_cause.pl [start_date] [end_date]
...
sub main
{
	...
	$sql = "delete from TSMD_MMSC_MISC_PER_CAUSE" 
	...
	$sql = "select area_code,DSMP_RESULT,count(*) from TSMC_DSMP,TSMS_START_GT" 
		...
		. " and (substr(to_char(DST_MSISDN),1,9) = start_gt) group by area_code,DSMP_RESULT";
	stat_num($sql);
	
	for (@$stat_area_total_array)
	{
		my ($area_code,$dsmp_result,$num);
		
		$area_code = $_->[0] ? $_->[0] : 999;
		$dsmp_result = $_->[1] ? $_->[1] : 0;
		$num = $_->[2] ? $_->[2] : 0;

		$sql = "insert into TSMD_MMSC_MISC_PER_CAUSE(START_TIME,AREA_CODE,RESULT,SIGNALLING_MSG_TYPE,"
				. "NUM,RATIO,NE_TYP)"
				. " values(to_date('$startdate','yyyy-mm-dd'),$area_code,$dsmp_result,1,"
				. "$num,0,$AREANETWORK)";
	...
	$sql = "select SIGNALLING_MSG_TYPE,area_code,sum(NUM) from TSMD_MMSC_MISC_PER_CAUSE" 
		. " where (START_TIME between to_date('" . $startdate . " 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date('" . $startdate . " 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
	...
	$stat_row_hash = {};
	$sql = "select SIGNALLING_MSG_TYPE,RESULT,sum(NUM) from TSMD_MMSC_MISC_PER_CAUSE" 
		. " where (START_TIME between to_date('" . $startdate . " 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date('" . $startdate . " 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
	...
	$sql = "select SIGNALLING_MSG_TYPE,sum(NUM) from TSMD_MMSC_MISC_PER_CAUSE" 
		. " where (START_TIME between to_date('" . $startdate . " 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date('" . $startdate . " 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
...
sub stat_rate
{
	...
	foreach my $key (keys %$stat_row_hash)
	{
		...
		if($total == 0 )
		{
			$sql = "update TSMD_MMSC_MISC_PER_CAUSE set RATIO = 0"
    		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
			. " and AREA_CODE=$areacode and SIGNALLING_MSG_TYPE= $gdrtype and NE_TYP=$ne_type";
		}
		else
		{
			$sql = "update TSMD_MMSC_MISC_PER_CAUSE set RATIO = (NUM / $total) * 1.0000"
    		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
			. " and AREA_CODE=$areacode and SIGNALLING_MSG_TYPE= $gdrtype and NE_TYP=$ne_type";
...
sub update_title
{
	...
	$sql = "update TSMD_MMSC_MISC_PER_CAUSE a set RESULT_TITLE = (select DESCRIPTION_CN from TSMS_MMS_STATIC_DATA b where a.RESULT>=b.BEGIN_ID and a.RESULT<=b.END_ID and b.title_id=160)"
  		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
