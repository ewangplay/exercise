############################################################################
#Desc: stat cdr mm1 mo fail per cause
#Usage:stat_cdr_mm1_mo_fcaa.pl [start_date] [end_date]
#Create by xie jiayou  2009/01/12
#Modify by wang xiaohui 2009/05/25 for optimizing framework
#Modify by wang xiaohui 2009/06/18 for adding log output
############################################################################
use Log::Log4perl;
use DBIx::Log4perl;
use Time::Local;
...
sub daily_dispose
{
	...
	$sql = "delete from TSMD_CDR_MM1_MO_FCAA" 
	...
	for (@$stat_area_total_array)
	{
		...
		my $sql = "insert into TSMD_CDR_MM1_MO_FCAA(START_TIME,AREA_CODE,RESULT,MMS_STATUS_CODE,"
	...
	$sql = "select MMS_STATUS_CODE,area_code,sum(FCAA_NUM) from TSMD_CDR_MM1_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
	...
	$stat_row_hash = {};
	$sql = "select area_code,sum(FCAA_NUM) from TSMD_CDR_MM1_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
		. " group by area_code";
	...
	$stat_row_hash = {};
	$sql = "select MMS_STATUS_CODE,RESULT,sum(FCAA_NUM) from TSMD_CDR_MM1_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
	...
	$sql = "select MMS_STATUS_CODE,sum(FCAA_NUM) from TSMD_CDR_MM1_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
	...
	# stat total fcaa num
	$stat_row_hash = {};
	$sql = "select sum(FCAA_NUM) from TSMD_CDR_MM1_MO_FCAA" 
		. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
		. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))";
...
sub stat_rate_by_area
{
	...
	foreach my $key (keys %$stat_row_hash)
	{
		...
		if($total == 0 )
		{
			$sql = "update TSMD_CDR_MM1_MO_FCAA set RATIO_IN_TOTAL = 0"
    				. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
				. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
		...
		{
			$sql = "update TSMD_CDR_MM1_MO_FCAA set RATIO_IN_TOTAL = (FCAA_NUM / $total) * 1.0000"
    				. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
				. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
				. " and AREA_CODE=$areacode and MMS_STATUS_CODE= $gdrtype and NE_TYPE=$ne_type";
...
sub stat_avg_rate_by_area
{
	...
	foreach my $key (keys %$stat_row_hash)
	{
		...
		if($total == 0 )
		{
			$sql = "update TSMD_CDR_MM1_MO_FCAA set AVG_RATIO_IN_TOTAL = 0"
    				. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
				. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
		...
		{
			$sql = "update TSMD_CDR_MM1_MO_FCAA set AVG_RATIO_IN_TOTAL = (FCAA_NUM / $total) * 1.0000"
    				. " where (START_TIME between to_date('$startdate 00:00:00','yyyy-mm-dd hh24:mi:ss')" 
				. " and to_date('$startdate 23:59:59','yyyy-mm-dd hh24:mi:ss'))"
				. " and AREA_CODE=$key and NE_TYPE=$ne_type";
...
sub update_title
{
	...
	$sql = "update TSMD_CDR_MM1_MO_FCAA a" 
